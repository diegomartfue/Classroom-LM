...
/**
 * Calls the Anthropic Claude API to get a real AI tutor response.
 * Passes the full conversation history for context-aware replies.
 * 
 * SETUP: Set VITE_ANTHROPIC_API_KEY in your .env file.
 * Example: VITE_ANTHROPIC_API_KEY=sk-ant-...
 * 
 * NOTE: For production, move this call to a backend endpoint to keep
 * your API key secure. This direct browser call is fine for development.
 */
async function callAnthropicAPI(
  ...
  const messages = [
    // Include prior conversation turns (skip the very first user message
    // since we'll append it as the current message below)
  ...
  const response = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 1024,
      system:
        'You are an expert AI tutor for an engineering education platform called LearnAI. ' +
        'You help students understand concepts in mechanics, thermodynamics, materials science, ' +
        'and related engineering topics. Be clear, concise, and educational. ' +
        'When explaining equations, define each variable. ' +
        'Encourage students and suggest follow-up questions when appropriate.',
      messages,
    }),
  });
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const sendMessageToAI = useCallback(async (conversationId: string, message: string) => {
    ...
    const responseText = await callAnthropicAPI(message, history);
